        ...
        *,
        ...
        cur.execute(
            ...
            INSERT OR REPLACE INTO successes(
                attempt_id, session_id, ts, gen, indiv, prompt,
                variant_count, best_path, style_score, nsfw_score, fitness
            ) VALUES(?,?,?,?,?,?,?,?,?,?,?)
            """,
            (
                attempt_id,
                session_id,
                int(time.time()),
                int(gen) if gen is not None else -1,
                int(indiv) if indiv is not None else -1,
                prompt,
                int(variant_count),
                str(best_path),
                float(style_score),
...
def save_and_score(
    ...
    if triplets:
        best_path, best_nsfw, best_style = triplets[0]
        best_fitness = round(w_style * best_style + w_nsfw * best_nsfw, 2)
        logger.log_success(
            session_id=session_id,
            attempt_id=str(meta_base.get("id", "")),
            prompt=final_prompt,
            gen=gen,
            indiv=indiv,
            variant_count=len(saved_paths),
            best_path=best_path,
            style_score=best_style,
            nsfw_score=best_nsfw,
